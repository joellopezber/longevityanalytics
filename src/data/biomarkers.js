/**
 * biomarkers.js
 * Datos de biomarcadores organizados por paquetes y add-ons
 * Incluye precios dinámicos calculados desde priceData.js
 */
...
const csvData = [
  ...
  { name: "PCR (hsCRP)", category: "Inflamación cardiovascular", group: "Essential · Prevenii", code: "B3170", essential: true, gender: "both", description: "Proteína C reactiva altamente sensible. Marcador de inflamación sistémica y riesgo cardiovascular." },
...
// Los precios se calculan dinámicamente basándose en los biomarcadores incluidos
const essentialBiomarkers = csvData.filter(item => item.essential);
...
export const essentialPackage = {
  ...
  getPricing: (gender = 'both') => {
    const pricing = calculatePackagePrice(essentialBiomarkers, gender, 'essential');
    return {
      testCount: pricing.testCount,
      price: Math.round(pricing.finalPrice),        // Precio final (Prevenii con descuentos)
      costPrice: Math.round(pricing.marketPrice),   // PVP referencial (Market)
...
export const getEssentialPackageForGender = (gender) => {
  ...
  const pricing = essentialPackage.getPricing(gender);
