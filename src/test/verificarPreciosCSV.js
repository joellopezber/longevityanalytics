...
const waitForPackages = () => {
  return new Promise((resolve, reject) => {
    ...
    const checkPackages = () => {
      attempts++;
      if (window.getPriceByCode) {
        resolve(true);
      } else if (attempts >= 10) {
        reject(new Error('Función getPriceByCode no disponible'));
      } else {
...
waitForPackages().then(() => {
  ...
  Object.keys(preciosCSV).forEach(codigo => {
    ...
    const sistemaPrevenii = window.getPriceByCode(codigo, 'prevenii');
    const sistemaMarket = window.getPriceByCode(codigo, 'market');
